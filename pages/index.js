...
// });
 ...
 return (
<div>
  <div className={styles.headBox}><h1 className={styles.header}>Squatch Trait Swap!</h1>
  </div>
     ...
     height="500"
      ...
      <div className={styles.components}>
        ...
        <select className={styles.select}
              type="number" min="1" max="3333"
              required
              value = {squaNum} 
              onChange={(e) => {addSquatch(e.target.value)}}
            >
              {data.map((data) => (
              <option key={data.name} value={data.name.slice(12)}>{data.name}</option>
            ))}
            </select>
            <br/>
            <br/>
            <button className={styles.button} onClick={clearSquatch}>{buttonText}</button>
        {/* <div><a href={canvasImage} target="_blank" rel="noreferrer noopener">Save Your Squatch</a></div> */}
    </div>
